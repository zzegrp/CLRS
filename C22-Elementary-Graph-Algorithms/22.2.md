...
### `Answer`
用邻接矩阵表示的话,遍历所有的边的时间由O(E)变为O(E*V),因此总运行时间是O(E*V)

If the graph is represented by adjancency matrix, then the time of iterating all the edge becomes O(E*V), so the running time is O(E*V).
